...
# Evaluate Mistral Large 2 Instruct (123B) base model on translation task.
...
PRED_BASE_FILE = os.getenv("PRED_MISTRAL_BASE_FILE", "resources/predictions_mistral_base.jsonl")

BASE_MODEL_PATH = os.getenv("MISTRAL_MODEL_PATH")

# Performance knobs
BATCH_EVAL = int(os.getenv("BATCH_EVAL", "4"))  # smaller batch for 123B model
MAX_NEW_TOKENS = int(os.getenv("MAX_NEW_TOKENS", "256"))

# Language code to full name mapping (ISO 639-1 codes used in EU documents)
LANG_NAMES = {
    "BG": "Bulgarian",
    "CS": "Czech",
    "DA": "Danish",
...
def load_model_and_tokenizer(base_model_path: str):
...
def main():
    ...
    print("\nLoading Mistral Large 2 Instruct (123B) base model...")
    tok, model = load_model_and_tokenizer(BASE_MODEL_PATH)
    ...
    with open(PRED_BASE_FILE, "w", encoding="utf-8") as f_out:
    ...
    # === EVALUATION METRICS ===
    print("\n" + "=" * 60)
    print("EVALUATION RESULTS — Mistral Large 2 Instruct (base)")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print(f"Predictions saved to: {PRED_BASE_FILE}")
